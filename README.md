...
Started as a Political Science major. Took one CS class, got hooked, and haven't stopped building since.

Unpaid internship at Routes (PHP/MySQL, 42% API latency reduction, horizontal scaling to 1k+ concurrent profiles) → Calstart (automated 60% of my own workflow, LangChain + Gemini embeddings) → CalHacks (VecSec, 98% malicious prompt detection at 40ms, 3M+ req/day) → Red Canyon (C++ telemetry and gRPC modules for NASA Core Flight Software) → Datadog incoming.

Now building autonomous robots.

---

## Connect

[LinkedIn](https://linkedin.com/in/ralphhyacinthe/) · [Email](mailto:RHyacinthe@colgate.edu)
